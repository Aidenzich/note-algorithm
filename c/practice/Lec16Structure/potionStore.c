// gcc potionStore.c -o ps && ./ps < ps.in
...
void sum_total_costs(order_t orders[], unsigned order_cnt)
{
    for (int i=0; i < order_cnt; i++){
        ...
        for (int j=0; j < orders[i].item_cnt; j++){
            total += orders[i].items[j].cnt * orders[i].items[j].cost;            
        }
...
int cmpOrder(const order_t a, const order_t b){
    int total_hm_t_a = a.pick_up_time.hour * 60 + a.pick_up_time.minute;
    int total_hm_t_b = b.pick_up_time.hour * 60 + b.pick_up_time.minute;
    if (total_hm_t_a > total_hm_t_b){
        return 1;
    }
    return 0;
}
...
int cmp(const void *a, const void *b){
    return cmpOrder(*(const order_t *)a, *(const order_t *)b);
}
